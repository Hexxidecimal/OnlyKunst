  ...
  if(isset($_GET["verzenden"]) && isset($_GET["search"]) && $_GET["search"] != ""){
    ...
    if($stmt = $mysql->prepare($sql)){
        ...
        if(!$stmt->execute()){
            ...
            $stmt->bind_result($activiteitId, $activiteitDatum, $activiteitNaam, $activiteitDescriptie, $activiteitDuur, $activiteitPlaatsId, $activiteitSoortId, $activiteitToegevoegd, $activiteitToegevoegdDoorId, $activiteitFotoURL);

            while($stmt->fetch()){
                $actDat[$activiteitId] = $activiteitDatum;
                $actNaa[$activiteitId] = $activiteitNaam;
                $actDes[$activiteitId] = $activiteitDescriptie;
                $actDuu[$activiteitId] = $activiteitDuur;
                $actPlI[$activiteitId] = $activiteitPlaatsId;
                $actToe[$activiteitId] = $activiteitToegevoegd;
                $actToI[$activiteitId] = $activiteitToegevoegdDoorId;
                $actFoU[$activiteitId] = $activiteitFotoURL;
                $searchCount++;

                switch($activiteitSoortId){
                  case 1:
                    $actSoo[$activiteitId] = "Markt";
                    break;
                  case 2:
                    $actSoo[$activiteitId] = "Tentoonstelling";
                    break;
                  case 3: 
                    $actSoo[$activiteitId] = "Workshop";
                }
            }
        }
        $stmt->close();
        ...
        <form class="container" id="searchBar" method="GET" style="width: 60%" action="<?php echo $_SERVER['PHP_SELF']; ?>">
          ...
          <div class="input-group mb-3">
            ...
            <div class="input-group-append">
              <input type="submit" class="btn btn-primary btn-lg" id="verzenden" name="verzenden" onclick="scroll()" value="Browse Activiteiten">
            </div>
          </div>
          ...
          if($searchCount > 0){
            ...
            echo "<div class='container-fluid container-lg'>
                    <div class='row h-100 g-2 justify-content-center'>";
                      ...
                      foreach($actNaa as $key => $pener){
                        ?>
                        
                          
                              <div class="card mb-3" style="max-width: 540px; margin: 0% 5% 0% 5%">
                                <div class="row no-gutters">
                                  <div class="col-md-4 d-none d-sm-block d-sm-none d-md-block">
                                    <img src="<?php echo $actFoU[$key] ?>" class="card-img img-fluid" alt="...">
                                  </div>
                                  <div class="col-md-8">
                                    <div class="card-body">
                                      <h5 class="card-title"><?php echo $actNaa[$key] ?></h5>
                                      <p class="card-text"><?php echo $actDes[$key] ?></p>
                                      <p class="card-text"><small class="text-muted"><?php echo $actSoo[$key] ?></small></p>
                                    </div>
                                  </div>
                                </div>
                              </div>
