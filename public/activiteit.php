...
.card button:hover {
  ...
  <body>
    ...
    <!-- ===============================================-->
        ...
        <div class="container"><a class="navbar-brand d-inline-flex" href="index.php"><img class="d-inline-block" src="assets/img/logo.png" alt="logo" /><span class="text-1000 fs-0 fw-bold ms-2">OnlyKunst</span></a>
          ...
          <div class="collapse navbar-collapse border-top border-lg-0 mt-4 mt-lg-0" id="navbarSupportedContent">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
              <li class="nav-item px-2"><a class="nav-link fw-medium active" aria-current="page" href="#">mainpage</a></li>
              <li class="nav-item px-2"><a class="nav-link fw-medium active" aria-current="page" href="ActiviteitenToevoegen.php">maak een nieuwe activiteit aan</a></li>
            </ul>
            <form class="d-flex">
      ...
      <form action="activiteit.php" method="post" enctype="multipart/form-data">
          ...
          <div class="row flex-center">
            <div class="col-12 mb-10">
              <div class="d-flex align-items-center flex-column">
                <h1 class="fw-normal">De leukste activiteiten</h1>
      ...
      <?php
        ...
        if(mysqli_connect_errno()){
            ...
            $sql="select ActiviteitDatum, activiteitFotoURL from tblactiviteiten";
            if($stmt = $mysqli->prepare($sql)){
                if(!$stmt->execute()){
                    echo'Het uitvoeren van de query is mislukt: '.$stmt->error.'in query';
                }
                else{
                    $stmt->bind_result($ActiviteitDatum,$activiteitFotoURL);
                    while($stmt->fetch()){
                      if($ActiviteitDatum < date("Y-m-d H:i:s"))
                       {
                          unlink($activiteitFotoURL);
                      }
                  }
                    }
                }
    ...
    ?>
      <?php 
            ...
            $sql = "DELETE from tblactiviteiten WHERE ActiviteitDatum < curdate()" ;
            if($stmt = $mysqli->prepare($sql))
            {
                 if($stmt->execute())
                 {
    ...
    ?>
      ...
      if(mysqli_connect_errno()){
        ...
        $sql="select ActiviteitId, ActiviteitNaam, ActiviteitDescriptie, ActiviteitSoortID, activiteitFotoURL from tblactiviteiten";
        if($stmt = $mysqli->prepare($sql)){
            if(!$stmt->execute()){
                echo'Het uitvoeren van de query is mislukt: '.$stmt->error.'in query';
            }
            else{
                $stmt->bind_result($ActiviteitId,$ActiviteitNaam,$ActiviteitDescriptie,$ActiviteitSoortID,$activiteitFotoURL);
                while($stmt->fetch()){
                    if($teller == 0)
                    {
                      echo "<div class=\"row\">";
                    ...
                    <img src=\"$activiteitFotoURL\" alt=\"fotoEvenement\" style=\"width:100%; height:275px;\">
                    <h1>$ActiviteitNaam</h1>
                    <p class=\"price\">"; if ($ActiviteitSoortID == 1){echo "Markt";}else if($ActiviteitSoortID == 2){echo "Tentoonstelling";}else{echo "Workshop";} echo "</p>
                    <p>$ActiviteitDescriptie</p>
