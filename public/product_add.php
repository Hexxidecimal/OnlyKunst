...
<?php 
  ...
  if(isset($_GET["verzenden"]) && isset($_GET["search"]) && $_GET["search"] != ""){
    ...
    $sql = "INSERT INTO tblartikel (ArtikelNaam, ArtikelFoto, ArtikelPrijs, ArtikelBeginprijs) VALUES (?,?,?,?)";
    
    if($stmt = $mysqli->prepare($sql))
        {
            $stmt->bind_param('ssii', $artikelNaam, $artikelFoto, $artikelPrijs, $artikelBeginPrijs);
            $artikelNaam=$mysqli->real_escape_string($_POST['titel']);
            $artikelFoto=$mysqli->real_escape_string($_POST['omschrijving']);
            $artikelPrijs=$mysqli->real_escape_string($_POST['price']);
            $artikelBeginPrijs=$mysqli->real_escape_string($_POST['price']);
  ...
  <body>
    ...
    <main class="main" id="top">
      ...
      <section class="py-11 bg-light-gradient border-bottom border-white border-5">
        <div class="bg-holder overlay overlay-light" style="background-image:url(assets/img/gallery/logoBack1.png);background-size:cover;">
        </div>
        <!--/.bg-holder-->
          ...
          <div class="col-md-12">
          <div class="bg-100 p-4 h-100 d-flex flex-column justify-content-center shadow">
          <div class="row">
            <h2 class="text-center">Voeg een werk toe</h2>
          </div>
            ...
            <form method="post" >
              ...
              <div class="input-group">
                ...
                <input type="text" name="price" id="price" class="form-control" placeholder="Prijs" aria-describedby="inputGroupPrepend2" required>
              </div>
            </div>
            <div class="col-md-4 mb-3">
              <label>Titel</label>
          ...
          <div class="row">
            <input type="submit" class="btn btn-primary" style="margin-top: 10%;">
          </div>
          </div>
          </div>
          </div>
